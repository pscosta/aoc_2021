...
fun day4() {
    ...
    var boards = input.asSequence().drop(2)
        .filterNot { it.isEmpty() }
        .chunked(5)
        .map { it.map { it.trim().replace("  ", " ").split(" ").map { it.toInt() } } }.toMutableList()
    ...
    run sol1@{
        allDrawNums.forEach { drawNum ->
            ...
            boards.firstOrNull { it.isWinner(drawnNums) }
                ?.also { println("sol1: ${it.score(drawNum, drawnNums)}").also { return@sol1 } }
        }
    }
    run sol2@{
        allDrawNums.forEach { drawNum ->
